...
* **Domain** - предметная область, описание бизнес-правил и их взаимодействие между собой. Не зависит и не должно зависеть ни от каких других слоев приложения. Является отражением какой-то бизнес-сущности, бизнес-процесса.
  ...
  * **Service** - обертка для агрегата, описывает все возможные взаимодействия с агрегатом, а также взаимодействие между разными сущностями внутри домена, могут быть описаны правила взаимодействия, какая-то логика, которая запускается на уровне Application. Из этого слоя могут триггериться доменные события.
...
## Value Object
Часть сущности, не имеющая идентификатора, но значение которой само по себе является значением. Может переиспользоваться в рамках контекста. Пример value object - координаты в виде пары значений долготы и широты.
...
## Todo
* Избавиться от AsMessageHandler в слое Application
* Кеширование данных
* Расширение Product
